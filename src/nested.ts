...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (sumCount: number, question: Question) => sumCount + question.points,
        0,
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions
        ...
        .reduce(
            (sumCount: number, question: Question) =>
                sumCount + question.points,
            0,
        );
}
...
export function duplicateQuestionInArray(
    ...
    const duplicate: Question = duplicateQuestion(newId, questions[index]);
